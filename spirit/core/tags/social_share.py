...
FACEBOOK_URL = "https://www.facebook.com/sharer.php?%s"
TWITTER_URL = "https://twitter.com/share?%s"
#GPLUS_URL = "https://plus.google.com/share?%s"
LINKEDIN_URL = "https://www.linkedin.com/sharing/share-offsite/?%s"                
...
def get_facebook_share_url(context, url, title):
    ...
    params = [('u',request.build_absolute_uri(url)),
              ('t',title)]
               #)),
              #('p[url]', ]#,
              #('p[title]', title)]
...
@register.simple_tag(takes_context=True)
def get_linkedin_share_url(context,url):
    request = context['request']
    params = [('url', request.build_absolute_uri(url)), ]
